...
from visual import *     #PROBLEM:  UNABLE TO IMPORT VPYTHON ON BBB
import threading, time, sys, traceback    #These should be okay

com_port = "COM12"                     #On BBB "/dev/ttyO#"
baudrate = 115200

# serial port
#ser = None                             #Why initiate if it is NONE?
...
scene.title = "LidarMap2"

class grid:

    def __init__(self, size=100, big_interval=50):
        self.frame = frame(pos=(0,0,0))
        for i in range(-size, size+big_interval, big_interval):
            if i %big_interval == 0:
                c = color.gray(0.65)
            #else:
                #c = color.gray(0.25)
...
my_grid = grid(size=6000, big_interval=1000)
#my_grid.frame.pos.y=-5        #not sure what this does. Seems to have no effect


## setup display

offset = 0                     # also not sure what this does yet
...
#draw the robot (very approximately)
robot = frame()
...
#draw the lidar (even more approximately)
lidar=frame()
...
#Text
#label_help = label(pos = (0,0,0))     #works without this.  
label_speed = label(pos = (0,-500,0), xoffset=1, box=False, opacity=0.1) #Does not work without this
label_errors = label(pos = (0,-1000,0), xoffset=1, text="errors: 0", visible = False, box=False)  #Does not work without this

# Different attributes to display
use_points = True
use_outer_line = True
use_lines = True
use_intensity = True
...
def update_view( angle, data ):
    ...
    ### Plotting stuff ###
    # Conditional for the use of outer lines
        ...
        try:
            ...
            if init_level == 0 :
                ...
                if checksum(all_data) == incoming_checksum:
                    ...
                    gui_update_speed(speed_rpm)  #update graph

                    #motor_control(speed_rpm)

                    update_view(index * 4 + 0, b_data0)   #update graph
                    update_view(index * 4 + 1, b_data1)   #update graph
                    update_view(index * 4 + 2, b_data2)   #update graph
                    update_view(index * 4 + 3, b_data3)   #update graph
                else:
...
from visual import *     #PROBLEM:  UNABLE TO IMPORT VPYTHON ON BBB
import thread, time, sys, traceback    #These whoudl be okay
